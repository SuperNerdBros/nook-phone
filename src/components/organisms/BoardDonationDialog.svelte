<script lang="ts">
  ...
  import { donateToBoard } from '@/lib/api';
  
  const { board, initialRaised, isFunded, onClose, onFunded } = $props<{ board: string; initialRaised: number; isFunded: boolean; onClose: () => void; onFunded?: () => void; }>();
  const ctx = getPhoneContext();
  
  const GOAL = 280000;
  let currentDonation = $state(initialRaised);
  let remaining = $derived(Math.max(0, GOAL - currentDonation));
  let bellsAvailable = $derived(nookState.bells);
  let wasBoardPermanent = $state(isFunded);
  ...
  $effect(() => {
    if (currentState === 'intro') {
      donationAmount = wasBoardPermanent ? Math.min(bellsAvailable, 10000) : Math.min(bellsAvailable, remaining);
    }
  });
  ...
  $effect(() => {
    if (currentState === 'intro') {
      if (wasBoardPermanent) {
        dialogText = `Hullo, hullo! The ${board} board is fully funded, but we're always accepting donations!`;
      } else {
        dialogText = `Hullo, hullo! We're raising funds to make the ${board} board a permanent resident! What do you say?`;
      }
      ...
      dialogText = `Huzzah! We've reached our funding goal! The ${board} board is now a permanent fixture! Thank you!`;
      isLloidDancing = true;
    }
  });
  ...
  async function handleDonate() {
    if (donationAmount > 0 && donationAmount <= bellsAvailable) {
      nookState.deductBells(donationAmount);
      const res = await donateToBoard(board, donationAmount);
      
      if (res && res.success) {
        currentDonation = res.raised;
            ...
            <div class="flex gap-2">
              <div class="flex-1 relative flex items-center">
                ...
                <input 
                  type="number" 
                  min="0" 
                  max={wasBoardPermanent ? bellsAvailable : Math.min(bellsAvailable, remaining)}
                ...
                />
                <button 
                  onclick={() => donationAmount = wasBoardPermanent ? bellsAvailable : Math.min(bellsAvailable, remaining)}
