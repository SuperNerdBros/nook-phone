<script lang="ts">
  ...
  function getGiftReaction(villager: any, item: any) {
    ...
    switch (stars) {
      case 1:
        if (personality === "Cranky")
          message = `Bah! A ${item.name}? What am I supposed to do with this garbage?!`;
        else if (personality === "Snooty")
          message = `Oh... a ${item.name}. Are you giving me your trash? How... delightful.`;
        else if (personality === "Jock")
          message = `Uh, thanks, but this ${item.name} is totally useless for my gains...`;
        else message = `Oh. A ${item.name}. Um... I guess I'll take it. Thank you?`;
        break;
      case 2:
        if (personality === "Cranky")
          message = `A ${item.name}? Well, it ain't much, but thanks anyway.`;
        else if (personality === "Peppy")
          message = `Oh! A ${item.name}! That's so random, but thanks bestie!`;
        else if (personality === "Lazy")
          message = `A ${item.name}? Wow, I didn't have to do anything to get this. Thanks!`;
        else message = `Oh, a ${item.name}. Thank you for the thought!`;
        break;
      case 3:
        if (personality === "Peppy")
          message = `Oh my gosh, a ${item.name}! That is so cute! Thanks a million!`;
        else if (personality === "Smug")
          message = `Ah, a ${item.name}. You have an exquisite eye for gifts, my friend.`;
        else message = `Wow, a ${item.name}! I really like this. Thank you so much!`;
        break;
      case 4:
        if (personality === "Jock")
          message = `Woah, a ${item.name}! This is awesome! Thanks for supporting my training!`;
        else if (personality === "Sisterly")
          message = `Hey, you really hit the nail on the head with this ${item.name}! Thanks!`;
        else message = `Oh, I love it! A ${item.name} is exactly what I wanted. Thank you!`;
        break;
      case 5:
        if (personality === "Lazy")
          message = `HOORAY! A ${item.name}! This is so amazing! Here, you should have something in return!`;
        else if (personality === "Snooty")
          message = `Oh my! A ${item.name}! This is incredibly luxurious. I simply must give you something back!`;
        else
          message = `Oh my goodness! A ${item.name}! I absolutely love this so much! Wait right here, I have a gift for you too!`;
        break;
    }
  ...
  const allSpecies = $derived([...new Set(allVillagers.map((v) => v.species))].sort());
  ...
  const filteredVillagers = $derived(
    (isAddMode ? allVillagers : myContactsList)
      .filter((v) => {
        if (!isAddMode) {
...
<div class="flex flex-col h-full bg-[#fdfcf2] relative">
  {#if selectedVillager}
    ...
    <!-- Detail View -->
    <div
      class="absolute inset-0 z-30 bg-[#fdfcf2] flex flex-col overflow-y-auto ac-scrollbar animate-fade-in"
    >
      <!-- Header -->
      <div
        class="flex items-center justify-between p-4 bg-[#9cc677] border-b border-black/10 sticky top-0 backdrop-blur-md z-40"
      >
        <button
          onclick={closeContact}
          class="flex items-center text-[#5c3a21] font-bold text-sm bg-white/50 px-3 py-1.5 rounded-full shadow-sm hover:bg-white active:scale-95 transition-all cursor-pointer z-10"
        >
          <ChevronLeft class="w-4 h-4 mr-1" /> Back
        </button>
        <span
          class="absolute left-1/2 -translate-x-1/2 font-black text-lg text-[#5c3a21] tracking-tight pointer-events-none"
          >{selectedVillager.personality} {selectedVillager.species}</span
        >
        <button
          onclick={() => toggleVip(selectedVillager.id)}
          class="text-[#f0b157] bg-white/50 p-2 rounded-full shadow-sm hover:bg-white active:scale-95 transition-all cursor-pointer z-10"
        >
          <Star
            class="w-5 h-5 {nookState.isBestFriend(selectedVillager.id) ? 'fill-current' : ''}"
          />
        </button>
      </div>
      ...
      >
        <div class="h-32 mb-2 relative group flex items-end justify-center">
          ...
          {:else}
            <div
              class="w-28 h-28 rounded-full border-4 border-white bg-[#e1d9be] flex items-center justify-center text-4xl"
            >
              🐾
            </div>
          {/if}
      ...
      <div class="flex justify-center gap-4 px-6 mb-8">
        ...
        >
          <div
            class="w-12 h-12 rounded-full bg-[#ff8a8a] text-white flex items-center justify-center shadow-md active:scale-95 transition-transform hover:bg-[#ff7575]"
          >
        ...
        <button
          onclick={() => messageContact(selectedVillager)}
          class="flex flex-col items-center gap-1 cursor-pointer"
        >
          <div
            class="w-12 h-12 rounded-full bg-[#8b3a3a] text-white flex items-center justify-center shadow-md active:scale-95 transition-transform hover:bg-[#6a2c2c]"
          >
        ...
        <button
          onclick={() => toggleSubscription(selectedVillager)}
          class="flex flex-col items-center gap-1 cursor-pointer"
        >
          <div
            class={`w-12 h-12 rounded-full text-white flex items-center justify-center shadow-md active:scale-95 transition-transform ${nookState.isSubscribed("bb/" + selectedVillager.name.replace(/\s+/g, "")) ? "bg-[#eb6a9d]" : "bg-[#e1d9be] hover:bg-[#d4ccb1]"}`}
          >
            <Bookmark class="w-6 h-6 fill-current" />
          </div>
          <span class="text-[10px] font-bold text-[#5c3a21]"
            >{nookState.isSubscribed("bb/" + selectedVillager.name.replace(/\s+/g, ""))
              ? "Subscribed"
              : "Subscribe"}</span
          >
        </button>
        <button class="flex flex-col items-center gap-1 opacity-50 cursor-not-allowed">
          <div
            class="w-12 h-12 rounded-full bg-[#6db3e6] text-white flex items-center justify-center shadow-md"
          >
      ...
      <div class="px-5 flex flex-col gap-3 pb-8">
        ...
        >
          <div class="flex items-center gap-3">
            <div
              class="w-10 h-10 rounded-xl bg-[#9cc677]/20 text-[#4a752c] flex items-center justify-center"
            >
              <Home class="w-5 h-5" />
            </div>
            <div>
              <p class="text-xs font-black text-[#8a7f66] uppercase tracking-wider">
                Island Status
              </p>
        ...
        <div class="bg-white rounded-2xl p-4 shadow-sm border-2 border-[#e1d9be]">
          ...
          <div class="grid grid-cols-2 gap-y-4 gap-x-2">
            <div>
              ...
              <p class="text-sm font-bold text-[#5c3a21]">
                {selectedVillager.birthday || "Unknown"}
              </p>
            </div>
            <div>
        ...
        <div class="flex flex-col gap-3">
          ...
          <div class="bg-white rounded-2xl p-4 shadow-sm border-2 border-[#e1d9be]">
            ...
            <div class="grid grid-cols-2 gap-3">
              <button
                onclick={() => toggleMilestone("talkedToday")}
                class="flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all cursor-pointer {ms.talkedToday
                  ? 'bg-[#6cd476]/10 border-[#6cd476] text-[#4ca454]'
              ...
              <button
                onclick={() => toggleMilestone("giftedToday")}
                class="flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all cursor-pointer {ms.giftedToday
          ...
          <div class="bg-white rounded-2xl p-4 shadow-sm border-2 border-[#e1d9be]">
            ...
            <div class="grid grid-cols-2 gap-3">
              <button
                onclick={() => toggleMilestone("hasPoster")}
                class="flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all cursor-pointer {ms.hasPoster
                  ? 'bg-[#6db3e6]/10 border-[#6db3e6] text-[#4a90e2]'
                  : 'bg-gray-50 border-gray-100 text-[#8a7f66]'}"
              >
              ...
              <button
                onclick={() => toggleMilestone("hasPhoto")}
                class="flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all cursor-pointer {ms.hasPhoto
        ...
        <button
          ...
          class="mt-4 py-3 rounded-2xl border-2 border-[#ff8a8a] text-[#d96666] font-bold flex items-center justify-center gap-2 hover:bg-[#ff8a8a]/10 transition-colors cursor-pointer active:scale-95"
        >
  ...
  {:else}
      ...
      {#snippet actions()}
        {#if isAddMode}
          <button
            ...
            class="text-white font-bold text-sm bg-black/10 px-3 py-1.5 rounded-full hover:bg-black/20 transition-colors cursor-pointer mr-1"
          >
            Done
          </button>
        {:else}
    ...
    <div class="flex-1 overflow-y-auto ac-scrollbar bg-[#fdfcf2] relative pb-28">
      {#if isLoading}
        <div class="flex flex-col items-center justify-center h-48 text-[#8a7f66]">
          <div
            class="w-8 h-8 border-4 border-[#9cc677] border-t-transparent rounded-full animate-spin mb-3"
          ></div>
          <p class="font-bold text-sm">Syncing Contacts...</p>
        </div>
      {:else if isAddMode && !searchQuery && !selectedSpecies && !viewAll}
        <div class="p-4 flex flex-col gap-5">
          <button
            onclick={() => (viewAll = true)}
            class="bg-[#9cc677] text-white py-3 px-6 rounded-2xl font-black shadow-sm hover:scale-[1.02] active:scale-95 transition-transform cursor-pointer w-full text-center"
          >
            View All Villagers
          </button>
          ...
          <div>
            <h3
              class="text-[#8a7f66] font-black text-xs uppercase mb-3 tracking-widest text-center"
            >
              Filter by Species
            </h3>
            <div class="grid grid-cols-5 gap-x-1 gap-y-1">
              {#each allSpecies as species}
                <button
                  onclick={() => (selectedSpecies = species)}
                  class="species-tile"
                >
                  <span class="text-5xl leading-none">{SPECIES_EMOJIS[species] || "🐾"}</span>
                  <span
                    class="text-[10px] font-black text-[#8a7f66] leading-tight truncate w-full text-center mt-1"
                    >{species}</span
                  >
                </button>
              {/each}
      ...
      {:else}
        {#if isAddMode && (selectedSpecies || viewAll)}
          <div
            class="px-4 py-3 bg-[#fdfcf2] flex items-center justify-between border-b border-[#e1d9be]"
          >
            ...
            <button
              ...
              class="text-xs font-bold text-[#ff8a8a] hover:underline cursor-pointer bg-white px-3 py-1 rounded-full border border-[#ff8a8a]/30 shadow-sm"
            >
              Clear Filter
            </button>
          </div>
        ...
        {#if !isAddMode && activeTab === "island"}
          <div
            class="px-2 sm:px-6 py-6 sm:py-8 h-full flex flex-col items-center justify-center min-h-[450px]"
          >
            <div
              class="grid grid-cols-3 gap-x-3 sm:gap-x-6 gap-y-8 sm:gap-y-10 w-full max-w-[340px]"
            >
              {#each Array(10) as _, index}
                ...
                {#if villager}
                  ...
                  <div
                    ...
                    class="flex flex-col items-center gap-2 sm:gap-3 relative transition-all duration-200 {index ===
                    9
                      ? 'col-start-2'
                      : ''} {draggedIndex === index ? 'opacity-30 scale-95' : ''} {dragOverIndex ===
                    index
                      ? 'scale-110'
                      : ''}"
                  >
                    <button
                      ...
                      class="flex flex-col items-center w-full transition-transform hover:scale-105 active:scale-95 cursor-pointer"
                    >
                      <div
                        class="w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-white shadow-md border-[3px] {dragOverIndex ===
                        index
                      ...
                      >
                        ...
                        {/if}
                        <div
                          class="absolute -bottom-2 inset-x-0 flex justify-center gap-0.5 sm:gap-1 pointer-events-none"
                        >
                          {#if ms.talkedToday}
                            <div
                              class="bg-[#6cd476] w-6 h-6 sm:w-7 sm:h-7 rounded-full border-2 sm:border-[3px] border-white flex items-center justify-center text-white shadow-sm"
                              title="Talked Today"
                            >
                          ...
                          {#if ms.giftedToday}
                            <div
                              class="bg-[#6db3e6] w-6 h-6 sm:w-7 sm:h-7 rounded-full border-2 sm:border-[3px] border-white flex items-center justify-center text-white shadow-sm"
                              title="Gifted Today"
                            >
                      ...
                      <span
                        class="text-xs sm:text-sm font-black text-[#5c3a21] leading-tight text-center w-full truncate px-1 mt-2 sm:mt-3"
                        >{villager.name}</span
                      >
                    </button>
                  </div>
                {:else}
                  <div
                    ...
                    ondragleave={() => (dragOverIndex === index ? (dragOverIndex = null) : null)}
                    ondrop={(e) => handleDrop(e, index)}
                    class="flex flex-col items-center gap-2 sm:gap-3 opacity-40 {index === 9
                      ? 'col-start-2'
                      : ''} transition-all duration-200 {dragOverIndex === index
                      ? 'scale-110 opacity-100'
                      : ''}"
                  >
                    <div
                      class="w-20 h-20 sm:w-24 sm:h-24 rounded-full border-[3px] border-dashed {dragOverIndex ===
                      index
                        ? 'border-[#6cd476] bg-[#6cd476]/10 ring-4 ring-[#6cd476]/30'
                        : 'border-[#e1d9be] bg-black/5'} flex items-center justify-center transition-all"
                    >
                      <Home
                        class="w-8 h-8 sm:w-10 sm:h-10 {dragOverIndex === index
                          ? 'text-[#6cd476]'
                          : 'text-[#8a7f66]'}"
                      />
                    </div>
                    <span
                      class="text-xs sm:text-sm font-bold {dragOverIndex === index
                        ? 'text-[#6cd476]'
                        : 'text-[#8a7f66]'}">Empty</span
                    >
                  </div>
                {/if}
              {/each}
        ...
        {:else}
                  ...
                  >
                    ...
                    >
                      ...
                      {#if !isAddMode && nookState.isResident(villager.id)}
                        <div
                          class="absolute -bottom-1 -right-1 bg-[#6cd476] w-4 h-4 rounded-full border-2 border-white"
                        ></div>
                      {/if}
                    </div>
                    <div class="flex-1 min-w-0 flex justify-between items-center">
                      <div class="flex flex-col pr-2">
                        <span class="font-black text-[#5c3a21] text-[15px] leading-tight truncate"
                          >{villager.name}</span
                        >
                        <span class="text-[11px] font-bold text-[#8a7f66] truncate"
                      ...
                      {#if isAddMode}
                        {#if nookState.isContact(villager.id)}
                          <div
                            class="w-6 h-6 rounded-full bg-[#6cd476] text-white flex items-center justify-center shrink-0"
                          >
                            <Check class="w-4 h-4" />
                          </div>
                        {:else}
                          <div
                            class="w-6 h-6 rounded-full border-2 border-[#e1d9be] text-[#e1d9be] flex items-center justify-center shrink-0"
                          >
                            <Plus class="w-4 h-4" />
                          </div>
                        {/if}
                      {:else}
                        <div class="flex items-center gap-1 shrink-0">
                          ...
                          <button
                            onclick={(e) => {
                              e.stopPropagation();
                              toggleVip(villager.id);
                            }}
                            class="w-7 h-7 rounded-full flex items-center justify-center transition-all hover:bg-black/5 active:scale-90 cursor-pointer border-0 bg-transparent p-0"
                            title={nookState.isBestFriend(villager.id)
                              ? "Remove Best Friend"
                              : "Add Best Friend"}
                          >
                            <Star
                              class={`w-4 h-4 transition-colors ${nookState.isBestFriend(villager.id) ? "text-[#f0b157] fill-current" : "text-[#e1d9be] hover:text-[#f0b157]"}`}
                            />
                          </button>
                        </div>
                      {/if}
          ...
          {#if filteredVillagers.length === 0}
            <div
              class="flex flex-col items-center justify-center h-48 text-[#8a7f66] px-8 text-center"
            >
              {#if isAddMode}
              ...
              {:else}
                ...
                <button
                  ...
                  class="bg-[#9cc677] text-white px-5 py-2.5 rounded-full font-bold shadow-md hover:bg-[#8bb268] transition-colors cursor-pointer active:scale-95"
                >
                  Add New Contact
                </button>
              {/if}
    ...
    {#if !isAddMode}
      <div
        class="absolute bottom-6 inset-x-4 bg-white/95 backdrop-blur-md border-2 border-[#e1d9be] rounded-3xl p-1.5 flex justify-between z-20 shadow-lg"
      >
        <button
          onclick={() => (activeTab = "all")}
          class="flex-1 flex flex-col items-center py-2 rounded-2xl transition-colors cursor-pointer {activeTab ===
          'all'
            ? 'bg-[#9cc677]/20 text-[#4a752c]'
            : 'text-[#8a7f66] hover:bg-gray-50 hover:text-[#5c3a21]'}"
        >
        ...
        <button
          ...
          class="flex-1 flex flex-col items-center py-2 rounded-2xl transition-colors cursor-pointer {activeTab ===
          'bestFriend'
            ? 'bg-[#f0b157]/20 text-[#d99c45]'
            : 'text-[#8a7f66] hover:bg-gray-50 hover:text-[#d99c45]'}"
        >
  ...
  {#if isGiftPickerOpen}
    <div class="absolute inset-0 z-50 bg-[#fdfcf2] flex flex-col overflow-hidden animate-fade-in">
      ...
      <div class="flex items-center justify-between p-4 relative z-40 shrink-0">
        <button
          onclick={() => (isGiftPickerOpen = false)}
          class="flex items-center text-[#5c3a21] font-bold text-sm bg-white/50 px-3 py-1.5 rounded-full shadow-sm hover:bg-white active:scale-95 transition-all cursor-pointer"
        >
      ...
      <div class="relative z-20 w-full px-4 pb-8 shrink-0 mt-auto">
        <AcnhBubble
          ...
          dialogText={giftReaction
            ? giftReaction.message
            : "Oh! Do you have a present for me? What is it, what is it?!"}
          isActive={true}
        ...
        >
          ...
          {#if !giftReaction || giftReaction.rating === 0}
            {#if isGiftLoading}
              <div class="mt-4 flex justify-center py-2">
                <div
                  class="w-6 h-6 border-4 border-[#9cc677] border-t-transparent rounded-full animate-spin"
                ></div>
              </div>
            {:else if giftSearchResults.length > 0}
              <div class="mt-4 border-t border-[#e1d9be]/60 pt-4 animate-fade-in">
                    ...
                    >
                      {#if item.imageUrl || item.image_url}
                        <img
                          ...
                          alt={item.name}
                          class="max-w-full max-h-full object-contain drop-shadow-sm p-1"
                          loading="lazy"
                        />
                      {:else}
                        <Leaf class="w-6 h-6 text-[#9cc677] mb-1" />
                        <span
                          class="text-[8px] font-bold text-[#8a7f66] leading-tight truncate w-full px-1"
                          >{item.name}</span
                        >
                      {/if}
                    </button>
                ...
                <button
                  onclick={() => (giftSearchResults = [])}
                  class="mt-2 text-xs font-bold text-[#ff8a8a] hover:underline w-full text-center"
                >
            ...
            {:else}
              <div
                class="mt-4 border-t border-[#e1d9be]/60 pt-4 flex flex-col gap-3 animate-fade-in"
              >
                <div class="flex gap-2">
                  ...
                  <button
                    ...
                    class="bg-[#9cc677] text-white px-4 py-2 rounded-xl font-bold shadow-sm hover:bg-[#8bb268] active:scale-95 transition-transform"
                  >
                    Search
                  </button>
                </div>
                <button
                  onclick={askWhatTheyWant}
                  class="bg-[#f0b157]/20 text-[#d99c45] border-2 border-[#f0b157]/40 px-4 py-2 rounded-xl font-bold text-sm shadow-sm hover:bg-[#f0b157]/30 active:scale-95 transition-transform flex items-center justify-center gap-2"
                >
          ...
          {:else}
            <!-- Final Rating -->
            <div
              class="mt-4 border-t border-[#e1d9be]/60 pt-4 flex flex-col items-center gap-3 animate-fade-in"
              style="animation-delay: 500ms;"
            >
              <div class="flex items-center gap-2 mb-2">
                <span class="text-[10px] font-black text-[#8a7f66] uppercase tracking-wider"
                  >Love Rating</span
                >
                <div class="flex gap-0.5">
                  {#each Array(5) as _, i}
                    <svg
                      class="w-5 h-5 transition-all {i < giftReaction.rating
                        ? 'text-[#ff6b6b] fill-current animate-bounce-short'
                        : 'text-gray-300'}"
              ...
              <button
                onclick={() => (isGiftPickerOpen = false)}
                class="bg-[#9cc677] text-white px-6 py-2.5 rounded-xl font-bold shadow-sm hover:bg-[#8bb268] active:scale-95 transition-transform w-full text-center cursor-pointer"
              >
                You're welcome!
              </button>
            </div>
