<script lang="ts">
  ...
  const toggle24Hour = () =>
    nookState.updateSettings({ use24HourTime: !nookState.settings.use24HourTime });
  const toggleMotion = () =>
    nookState.updateSettings({ reduceMotion: !nookState.settings.reduceMotion });
  const toggleSound = () =>
    nookState.updateSettings({ soundEffects: !nookState.settings.soundEffects });
  ...
  async function handlePurchase() {
    ...
    if (nookState.bells >= WALLPAPER_PRICE) {
      ...
      const tx = await processTransaction(
        -WALLPAPER_PRICE,
        `Wallpaper: ${purchaseDialogWallpaper?.name}`
      );
  ...
  const patternWallpapers = ALL_WALLPAPERS.filter(
    (wp) => !officialWallpaperIds.has(wp.id) && !fakeArtWallpaperIds.has(wp.id)
  );
  ...
  const categories = [
    {
      ...
      description: "Wallpapers, custom designs",
      icon: ImageIcon,
      color: "bg-[#fdafb2]"
    },
    {
      ...
      description: "Time format, sounds, motion, grid size",
      icon: Smartphone,
      color: "bg-[#7cb988]"
    },
    {
      id: "apps",
      name: "Default Apps",
      description: "Choose default apps for actions",
      icon: LayoutList,
      color: "bg-[#ebd478]"
    },
    {
  ...
  const INTENT_CATEGORIES = [
    {
      ...
      filter: (app: any) =>
        app.id === "miles" ||
        app.name?.toLowerCase().includes("miles") ||
        app.information?.includes("gameplay")
    },
    {
      id: "critter",
      name: "Critterpedia",
      icon: "critter",
      filter: (app: any) => app.id === "critter" || app.information?.includes("critters")
    },
    {
      id: "shopping",
      name: "Nook Shopping",
      ...
      filter: (app: any) =>
        app.id === "shopping" ||
        app.information?.includes("items") ||
        app.tools?.includes("marketplace")
    },
    {
      id: "diy",
      name: "DIY Recipes",
      icon: "diy",
      filter: (app: any) => app.id === "diy" || app.information?.includes("items")
    },
    { id: "map", name: "Island Map", icon: "map", filter: (app: any) => app.id === "map" },
    {
      ...
      icon: "designer",
      filter: (app: any) => app.id === "designer" || app.tools?.includes("simulator")
    },
...
<div class="h-full flex flex-col font-['Varela_Round',sans-serif] nook-settings-bg">
  ...
  >
    {#snippet iconSnippet()}
      <img
        src={resolveAssetUrl(settingsIcon)}
        alt="Settings"
        class="w-16 h-16 object-contain drop-shadow-md rounded-[1.2rem]"
      />
    {/snippet}
  ...
  <div class="flex flex-1 overflow-hidden relative bg-transparent">
    ...
    {#if currentView === "main"}
      <div
        class="flex-1 overflow-y-auto ac-scrollbar flex flex-col pt-4 pb-12 px-4 gap-3 w-full h-full absolute inset-0"
      ...
      >
        {#each categories as cat}
          ...
          >
            <div
              class={`w-10 h-10 rounded-full ${cat.color} flex items-center justify-center text-[#5c3a21] shrink-0 shadow-sm`}
            >
    ...
    {:else}
      <!-- Subpages Area -->
      <div
        class="flex-1 overflow-y-auto ac-scrollbar p-4 sm:p-6 bg-transparent shadow-inner w-full h-full absolute inset-0"
        in:fly={{ x: 20, duration: 250 }}
        out:fly={{ x: 20, duration: 250 }}
      >
        <div class="max-w-3xl mx-auto flex flex-col gap-6">
          {#if currentView === "personalization"}
            ...
            <div class="flex flex-col gap-4 animate-fade-in pb-20">
              ...
              <!-- Category Tabs -->
              <div
                class="flex gap-2 p-1 bg-[#4a2e19]/10 rounded-full border-2 border-[#e1d9be]/30 self-center my-2 shrink-0"
              >
                <button
              ...
              <div class="flex flex-wrap justify-center gap-4 pb-6 px-1">
                {#each activeWallpaperTab === "official" ? officialWallpapers : activeWallpaperTab === "patterns" ? patternWallpapers : fakeArtWallpapers as wp}
                  ...
                  >
                    <div
                      class={`w-full aspect-[9/16] rounded-2xl border-4 shadow-sm relative overflow-hidden transition-all ${nookState.activeWallpaperId === wp.id || (!nookState.activeWallpaperId && wp.id === "default") ? "border-[#6cd476] ring-4 ring-[#6cd476]/30" : "border-[#e1d9be]"}`}
                    >
                      {#if wp.isDefault}
                        <div class={`absolute inset-0 ${wp.bg}`}></div>
                        <div
                          class={`absolute inset-0 opacity-40 ${wp.patternClass}`}
                          style={`mask-image: url('data:image/svg+xml,%3Csvg width=%2240%22 height=%2240%22 viewBox=%220 0 40 40%22 xmlns=%22http://www.w3.org/2000/svg%22%3E%3Cpath d=%22M20 5l5 10h10l-8 7 3 10-10-6-10 6 3-10-8-7h10z%22/%3E%3C/svg%3E'); -webkit-mask-image: url('data:image/svg+xml,%3Csvg width=%2240%22 height=%2240%22 viewBox=%220 0 40 40%22 xmlns=%22http://www.w3.org/2000/svg%22%3E%3Cpath d=%22M20 5l5 10h10l-8 7 3 10-10-6-10 6 3-10-8-7h10z%22/%3E%3C/svg%3E'); mask-size: 20px 20px; -webkit-mask-size: 20px 20px;`}
                        ></div>
                      {:else}
                        <img
                          src={wp.url}
                      ...
                      {#if nookState.activeWallpaperId === wp.id || (!nookState.activeWallpaperId && wp.id === "default")}
                        <div
                          class="absolute -bottom-1 -right-1 w-6 h-6 bg-[#6cd476] rounded-full border-2 border-white flex items-center justify-center shadow-sm z-30"
                        >
                      ...
                      {#if !freeWallpapers.has(wp.id) && !nookState.catalog.purchasedIds.includes(wp.id)}
                        <div
                          class="absolute inset-0 bg-black/30 flex items-center justify-center backdrop-blur-[1px] z-20"
                        >
                          <Lock
                    ...
                    <span
                      class="text-[10px] sm:text-[11px] font-black text-[#5c5446] text-center leading-tight"
                      >{wp.name}</span
                    >
                  </button>
                {/each}
                ...
                {#if activeWallpaperTab === "patterns"}
                  ...
                  >
                    <div
                      class="w-full aspect-[9/16] rounded-2xl border-4 border-dashed border-[#e1d9be] shadow-sm flex items-center justify-center bg-[#fdfcf9] text-[#caa253]"
                    >
                      <PaintBucket class="w-6 h-6" />
                    </div>
                    <span
                      class="text-[10px] sm:text-[11px] font-black text-[#5c5446] text-center leading-tight"
                      >Custom<br />Design</span
                    >
                  </button>
                {/if}
              </div>
          ...
          {#if currentView === "device"}
            ...
            <div class="flex flex-col gap-4 animate-fade-in">
              ...
              <div
                class="bg-white rounded-3xl p-5 border-4 border-[#e1d9be] shadow-[0_4px_0_#dcd3be] flex flex-col gap-2"
              >
                <div
                ...
                >
                  <div class="flex items-center gap-4">
                    <div
                      class="w-10 h-10 rounded-2xl bg-[#77a1e6] border-2 border-[#5b85ca] flex items-center justify-center text-[#5c3a21] shadow-inner"
                    >
                      <Clock class="w-5 h-5" />
                    </div>
                    <div>
                  ...
                  >
                    <div
                      class={`w-5 h-5 rounded-full bg-white shadow-sm transition-transform ${nookState.settings.use24HourTime ? "translate-x-5" : "translate-x-0"}`}
                    ></div>
                  </button>
                </div>
                ...
                >
                  <div class="flex items-center gap-4">
                    <div
                      class="w-10 h-10 rounded-2xl bg-[#fdafb2] border-2 border-[#e19396] flex items-center justify-center text-[#5c3a21] shadow-inner"
                    >
                  ...
                  >
                    <div
                      class={`w-5 h-5 rounded-full bg-white shadow-sm transition-transform ${nookState.settings.reduceMotion ? "translate-x-5" : "translate-x-0"}`}
                    ></div>
                  </button>
                </div>
                ...
                >
                  <div class="flex items-center gap-4">
                    <div
                      class="w-10 h-10 rounded-2xl bg-[#e89a78] border-2 border-[#cc7d5c] flex items-center justify-center text-[#5c3a21] shadow-inner"
                    >
                  ...
                  >
                    <div
                      class={`w-5 h-5 rounded-full bg-white shadow-sm transition-transform ${nookState.settings.soundEffects ? "translate-x-5" : "translate-x-0"}`}
                    ></div>
                  </button>
                </div>
                ...
                <div class="flex items-center justify-between py-3">
                  <div class="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div class="text-[10px] font-bold text-[#8a816f]">
                        Rows on homescreen ({nookState.settings.gridSize || 3})
                      </div>
                    </div>
                  </div>
                  <div
                    class="flex items-center gap-2 bg-[#f4f2e8] p-1 rounded-full border-2 border-[#e1d9be]"
                  >
                    {#each [3, 4, 5] as size}
                      <button
                ...
                <div class="flex items-center justify-between py-3">
                  <div class="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div class="text-[10px] font-bold text-[#8a816f]">
                        Capacity: {nookState.maxAppSlots || 18} / 54 slots
                      </div>
                    </div>
                  </div>
                  <div class="flex items-center gap-2">
                    ...
                    {:else}
                      <span
                        class="text-[#8a816f] font-black text-[10px] px-3 py-1.5 bg-[#f4f2e8] rounded-full border border-[#e1d9be]"
                        >Maxed Out</span
                      >
                    {/if}
                  </div>
          ...
          {#if currentView === "apps"}
            ...
            <div class="flex flex-col gap-4 animate-fade-in pb-20">
              ...
              <div class="text-[11px] font-bold text-[#8a816f] mb-2 leading-relaxed">
                Choose which apps to launch when you click on core icons or links. Built-in apps and
                installed community apps will appear here if they support the feature.
              </div>

              <div
                class="bg-white rounded-3xl p-5 border-4 border-[#e1d9be] shadow-[0_4px_0_#dcd3be] flex flex-col gap-0 divide-y-2 divide-dashed divide-[#f4f2e8]"
              >
                {#each INTENT_CATEGORIES as intent}
                  {@const eligibleApps = getEligibleApps(intent.id)}
                  {@const currentAppId =
                    (nookState.settings.defaultApps && nookState.settings.defaultApps[intent.id]) ||
                    intent.id}
                  {@const currentApp =
                    phone.allApps.find((a: any) => (a.id || a.name) === currentAppId) ||
                    phone.allApps.find((a: any) => a.id === intent.id)}

                  <div class="py-4 first:pt-1 last:pb-1 flex flex-col gap-2">
                    <div class="flex items-center justify-between">
                      <div class="flex items-center gap-4">
                        <div
                          class="w-10 h-10 rounded-2xl bg-[#fdfcf9] border-2 border-[#e1d9be] flex items-center justify-center shadow-inner overflow-hidden relative"
                        >
                          {#if currentApp?.image}
                            <img
                              src={resolveAssetUrl(currentApp.image)}
                              alt={intent.name}
                          ...
                          {:else if currentApp?.id}
                            <NookIcon
                              name={currentApp.id}
                              class="w-7 h-7 object-contain drop-shadow-sm p-1"
                            />
                          {:else if currentApp?.appIcon}
                            <NookIcon
                              name={currentApp.appIcon}
                              class="w-7 h-7 object-contain drop-shadow-sm p-1"
                            />
                          {:else}
                            <div class="font-black text-[#5c5446] text-[10px]">
                              {currentApp?.name?.charAt(0) || intent.name.charAt(0)}
                            </div>
                          {/if}
                        </div>
                        <div>
                          ...
                          <div class="text-[10px] font-bold text-[#8a816f]">
                            Default: {currentApp?.name || "System Default"}
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    {#if eligibleApps.length > 1}
                      <div class="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {#each eligibleApps as app}
                          ...
                          >
                            <div
                              class={`w-6 h-6 rounded-lg flex items-center justify-center shrink-0 ${app.bg || "bg-[#fdfcf9]"} overflow-hidden relative shadow-inner border-2 border-[#e1d9be]`}
                            >
                              {#if app.image}
                                <img
                                  src={resolveAssetUrl(app.image)}
                                  alt={app.name}
                                  class="w-4 h-4 object-contain drop-shadow-sm"
                                />
                              {:else if app.id}
                                <NookIcon
                                  name={app.id}
                                  class="w-4 h-4 object-contain drop-shadow-sm p-0.5"
                                />
                              {:else if app.appIcon}
                                <NookIcon
                                  name={app.appIcon}
                            ...
                            <span
                              class={`text-[10px] font-black truncate flex-1 ${currentAppId === (app.id || app.name) ? "text-[#4ca454]" : "text-[#8a816f]"}`}
                              >{app.name}</span
                            >
                            {#if currentAppId === (app.id || app.name)}
                              <div
                                class="w-4 h-4 rounded-full bg-[#6cd476] text-white flex items-center justify-center shadow-sm"
                              >
                                <svg
                                  class="w-3 h-3"
                                  fill="none"
                    ...
                    {:else}
                      <div class="mt-1 text-[10px] font-bold text-[#caa253]/80 italic">
                        No alternative apps installed for this category.
                      </div>
                    {/if}
                  </div>
                {/each}
          ...
          {#if currentView === "about"}
            ...
            <div class="flex flex-col gap-4 animate-fade-in pb-20">
              ...
              <div
                class="bg-white rounded-3xl p-8 border-4 border-[#e1d9be] shadow-[0_4px_0_#dcd3be] flex flex-col items-center justify-center gap-4 text-center"
              >
                <div
                  class="w-24 h-24 bg-[#f0b157]/20 rounded-[2rem] flex items-center justify-center border-4 border-[#f0b157]/40 shadow-inner relative overflow-hidden rotate-3"
                >
                  <div
                    class="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-30"
                  ></div>
                  <img
                    src="/wp-content/plugins/xophz-nook-phone/public/dist/nook-face.svg"
                    alt="Tom Nook"
                    class="w-16 h-16 drop-shadow-md z-10"
                  />
                </div>

                <div>
                  <h3 class="text-2xl font-black text-[#5c3a21] m-0">NookPhone<sup>1</sup></h3>
                  <p class="text-[10px] font-bold text-[#caa253] m-0 tracking-widest uppercase">
                    Version {(window as any).wpApiSettings?.version || "1.0.0"}
                  </p>
                </div>

                <div class="text-xs font-bold text-[#8a816f] max-w-xs mt-2 leading-relaxed">
                  NookOS is the premier operating system for island living. Designed by Nook Inc.
                  for a seamless and comfortable getaway package experience.
                </div>
                ...
                <div
                  class="mt-4 pt-4 border-t-2 border-dashed border-[#e1d9be] w-full text-[9px] font-bold text-[#8a816f] leading-relaxed"
                >
                  Animal Crossing™: New Horizons, Dodo Code™, and Dodo Airlines are trademarks of
                  Nintendo Co., Ltd.<br /><br />
  ...
  {#if isStorageUpgradeDialogOpen}
    <div
      class="absolute inset-0 z-[100] bg-black/40 backdrop-blur-sm flex flex-col overflow-hidden animate-fade-in"
    ...
    >
      <div class="absolute inset-0 flex items-center justify-center z-10 pointer-events-none pb-32">
        {#if isStoragePurchasing}
          <div
            class="animate-bounce flex items-center gap-2 text-[#ffdf28] font-black text-3xl drop-shadow-lg"
          >
      ...
      <div class="relative z-20 w-full px-4 pb-8 shrink-0 mt-auto">
        <AcnhBubble
          ...
          badgeBg="#d99c45"
          badgeColor="#ffffff"
          dialogText={storagePurchaseSuccess
          ...
          <div class="mt-4 border-t border-[#e1d9be]/60 pt-4 flex flex-col gap-3 animate-fade-in">
            {#if storagePurchaseSuccess}
              <button
                ...
                class="bg-[#1ca349] text-white px-4 py-2.5 rounded-xl font-bold shadow-sm hover:bg-[#188a3e] active:scale-95 transition-transform w-full text-center cursor-pointer border-0"
              >
                Awesome!
              </button>
            {:else if storagePurchaseError || nookState.bells < STORAGE_UPGRADE_PRICE}
              <button
                onclick={closeStorageDialog}
                class="bg-white border-2 border-[#e1d9be] text-[#8a7f66] px-4 py-2.5 rounded-xl font-bold shadow-sm hover:bg-gray-50 active:scale-95 transition-transform text-center cursor-pointer w-full"
              >
                Nevermind
              </button>
            {:else}
              <div class="flex gap-2">
                <button
                  ...
                  disabled={isStoragePurchasing}
                  class="flex-[2] border-0 bg-[#1ca349] disabled:opacity-50 text-white px-4 py-2.5 rounded-xl font-bold shadow-sm hover:bg-[#188a3e] active:scale-95 transition-transform text-center cursor-pointer"
                >
