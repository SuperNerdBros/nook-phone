...
<div
  class="h-full flex flex-col font-['Varela_Round'] bg-[#d9f6af] relative overflow-hidden select-none"
>
  <NookAppHeader
    title="ABD"
  ...
  >
    ...
    {#snippet actions()}
      <div
        class="bg-[#c8985c] rounded-full border-4 border-white px-3 py-1 flex items-center shadow-sm mr-2"
      >
        <img
          ...
          onerror={(e) => {
            (e.currentTarget as HTMLImageElement).src =
              'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="white"><path d="M12 2C9.5 2 7.5 4 7.5 6.5C7.5 7.4 7.8 8.2 8.3 8.9C6.3 10.5 5 13 5 16C5 19.9 8.1 23 12 23C15.9 23 19 19.9 19 16C19 13 17.7 10.5 15.7 8.9C16.2 8.2 16.5 7.4 16.5 6.5C16.5 4 14.5 2 12 2Z"/></svg>';
          }}
        />
        <span class="text-white font-bold text-lg leading-none mt-0.5"
          >{formatBells(nookState.bells)}</span
        >
      </div>
      <NookToolbarButton
  ...
  <div class="flex-1 w-full relative flex flex-col items-center pt-6">
    ...
    <div class="absolute inset-0 z-0 pointer-events-none flex flex-col justify-between opacity-60">
      <svg
        ...
        class="w-full h-[15%] text-[#ecfcce] fill-current"
      >
      ...
      <svg
        viewBox="0 0 100 20"
        preserveAspectRatio="none"
        class="w-full h-[15%] text-[#ecfcce] fill-current rotate-180"
      >
        <path d="M0,0 L100,0 L100,10 Q50,20 0,10 Z" />
      </svg>
    </div>
    ...
    <div
      ...
      style="background-image: radial-gradient(circle, #000 2px, transparent 2.5px); background-size: 30px 30px;"
    ></div>

    <p
      class="text-[#499a22] text-sm text-center font-bold px-4 max-w-[280px] leading-tight mb-4 z-10"
    >
    ...
    <div
      class="w-[90%] bg-white rounded-[40px] px-6 py-6 mt-2 shadow-sm z-10 border border-[#e2e2e2]"
    >
      <!-- Loan Balance -->
      <div class="flex justify-between items-end border-b-2 border-[#e6f9cd] pb-2 mb-3">
        <span
          class="{animatedLoan.current > 0
            ? 'text-[#e05638]'
            : 'text-[#c1c1c1]'} font-bold text-[15px]">Service Plan Balance</span
        >
        <div class="flex items-end">
          <span
            class="{animatedLoan.current > 0
              ? 'text-[#e05638]'
              : 'text-[#c1c1c1]'} font-bold text-2xl leading-none"
            >{formatBells(animatedLoan.current)}</span
          >
          <span
            class="{animatedLoan.current > 0
              ? 'text-[#e05638]'
              : 'text-[#c1c1c1]'} font-bold text-xs ml-1 mb-[2px]">Bells</span
          >
        </div>
      </div>
      ...
      <div class="flex justify-between items-end pt-1">
        ...
        <div class="flex items-end">
          <span class="text-[#499a22] font-bold text-2xl leading-none"
            >{formatBells(animatedBells.current)}</span
          >
    ...
    <div class="flex flex-col gap-4 mt-10 w-[75%] z-10">
      ...
      {#if nookState.servicePlanBalance > 0}
        ...
        >
          ...
          <div
            class="absolute inset-0 rounded-full border-[3px] border-[#c84022] opacity-50 m-[2px] pointer-events-none"
          ></div>
          ...
          <div class="relative h-full flex items-center justify-center gap-2">
            <svg
              ...
              /></svg
            >
            <span class="text-[#681604] font-extrabold text-[17px] tracking-wide mt-0.5"
              >Pay Service Plan</span
            >
          </div>

          <!-- Pointing Hand Hover Effect -->
          <div
            class="absolute left-[-15px] top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 group-hover:translate-x-2 transition-all duration-200 pointer-events-none"
          >
      ...
      <!-- Deposit Button -->
      <button
        class="hidden group relative w-full h-[52px] rounded-full overflow-hidden shadow-sm transition-transform active:scale-95"
      ...
      >
        ...
        <div
          class="absolute inset-0 rounded-full border-[3px] border-[#72bc20] opacity-50 m-[2px] pointer-events-none"
        ></div>
        ...
        <div class="relative h-full flex items-center justify-center gap-2">
          <svg
            ...
            stroke-linecap="round"
            stroke-linejoin="round"
            class="opacity-80"><path d="M12 2v14M8 12l4 4 4-4M3 20h18" /></svg
          >
      ...
      <!-- Withdrawal Button -->
      <button
        class="hidden group w-full h-[52px] rounded-full overflow-hidden shadow-sm transition-transform active:scale-95 relative"
      ...
      >
        ...
        <div class="relative h-full flex items-center justify-center gap-2">
          <svg
            ...
            stroke-linecap="round"
            stroke-linejoin="round"
            class="opacity-80"><path d="M12 22V8M8 12l4-4 4 4M3 4h18" /></svg
          >
          <span class="text-[#25510e] font-extrabold text-[17px] tracking-wide mt-0.5"
            >Withdrawal</span
          >
        </div>
      </button>
...
<style>
  ...
  @keyframes abdMoveStripes {
    ...
    100% {
      background-position: 42.426px 42.426px;
    }
  }
</style>
