<script lang="ts">
  ...
  import {
    TITLE_PART_1 as TITLE_PREFIXES,
    TITLE_PART_2 as TITLE_SUFFIXES
  } from "@/lib/passportTitles";
  ...
  let dialogType = $state<"none" | "need-pro" | "min-limit" | "confirm-delete" | "delete-failed">(
    "none"
  );
  ...
  let walletPassports = $derived(
    nookState.passports.map((p) => ({
      id: p.id,
  ...
  async function proceedDelete(id: string | number) {
    if (isPro && id !== "local") {
      syncing = true;
      const success = await deletePassport(id);
      syncing = false;
      if (!success) {
        dialogType = "delete-failed";
        return;
      }
    }
...
{#snippet passportCard(passport, isActive)}
  <div
    class="relative w-[95vw] max-w-[95vw] mx-auto shrink-0 snap-center rounded-[2rem] overflow-hidden transition-all duration-300 {isActive
      ? 'scale-100 shadow-xl'
      : 'scale-95 opacity-70 hover:opacity-100 cursor-pointer shadow-md'}"
  ...
  >
    ...
    <div class="px-5 py-4 flex flex-col h-full relative">
      <!-- Top header line -->
      <div
        class="flex items-center justify-center gap-3 mb-3 text-[#9b968a] font-bold text-[10px] tracking-widest uppercase"
      >
      ...
      <div
        class="bg-[#dfdbde] rounded-3xl p-4 flex-1 flex gap-4 relative overflow-hidden shadow-inner border border-white/40"
      >
        <!-- Optional pattern overlay (can be added via CSS if needed, using radial gradients or repeating SVGs) -->
        <div
          class="absolute inset-0 opacity-20 pointer-events-none"
          style="background-image: radial-gradient(circle at 10px 10px, #fff 2px, transparent 0); background-size: 20px 20px;"
        ></div>
        ...
        <div class="flex flex-col items-center relative z-10 w-28 shrink-0">
          <div class="w-24 h-24 rounded-3xl bg-[#fdfaf2] p-1.5 shadow-sm relative overflow-visible">
            ...
            <!-- Resident Rep Badge -->
            <div
              class="absolute -bottom-3 -left-3 -right-3 bg-[#fdfaf2] border-[3px] border-[#8a7258] rounded-lg py-1 px-2 text-[#5a4838] text-[9px] font-bold shadow-md flex items-center justify-center gap-1 -rotate-2"
            >
              <div
                class="w-3 h-3 bg-[#8a7258] text-[#fdfaf2] rounded-sm flex items-center justify-center"
              >
                <svg
                  ...
                  viewBox="0 0 24 24"
                  fill="currentColor"
                  stroke="none"><path d="M12 2L2 22h20L12 2z" /></svg
                >
              </div>
              Resident Rep.
            </div>
            ...
            {#if isActive && isPro}
              <Cloud
                class="w-5 h-5 absolute top-1 right-1 text-white bg-[#7cb988] rounded-full p-0.5 shadow-sm border border-white"
              />
            {/if}
          </div>
          ...
          {#if isActive}
            <button
              ...
              class="mt-6 bg-[#8a7258]/10 hover:bg-[#8a7258]/20 text-[#8a7258] p-1.5 rounded-full text-[9px] font-bold transition flex items-center gap-1"
        ...
        <div class="flex flex-col flex-1 relative z-10 mt-1 gap-2.5">
          <!-- Comment Bubble -->
          <div
            class="relative bg-[#fffce0] text-[#5c5643] text-xs font-medium px-3 py-1.5 rounded-2xl shadow-sm self-start inline-block max-w-full"
          >
            {passport.data.comment || "..."}
            <div
              class="absolute w-3 h-3 bg-[#fffce0] -left-1 top-2.5 rotate-45 transform origin-center shadow-sm -z-10"
            ></div>
            <div
              class="absolute w-3 h-3 bg-[#fffce0] -left-1 top-2.5 rotate-45 transform origin-center z-10"
            ></div>
          </div>
          ...
          <div class="flex items-center gap-2 text-[11px] font-bold text-[#353026] mt-1">
            <span class="flex items-center gap-1"
              ><span class="text-green-600 text-[10px]">🏝️</span> {passport.data.islandName}</span
            >
          </div>

          <!-- Title -->
          <div class="text-[11px] font-medium text-[#464032]">
            {passport.data.titlePrefix}
            {passport.data.titleSuffix}
          </div>
          ...
          <div class="flex items-center gap-1.5 text-[10px] font-medium text-[#7a7469] mt-auto">
            <span
              class="bg-[#b3a8d1] text-white rounded-full w-4 h-4 flex items-center justify-center text-[8px] font-bold shadow-sm"
              >DA</span
            >
        ...
        <!-- Watermark / Stamp -->
        <div
          class="absolute right-[-20px] bottom-[-10px] opacity-20 pointer-events-none scale-150 transform"
        >
          <svg
            width="100"
            height="100"
      ...
      {#if isActive}
        <div class="absolute bottom-2 right-4 flex items-center gap-4 z-20">
          ...
          <button
            onclick={(e) => {
              e.stopPropagation();
              isEditing ? handleSave() : (isEditing = true);
            }}
...
<div
  id="passport-app"
  class="flex flex-col h-full ac-app-screen"
>
  <NookAppHeader
  ...
  <div
    class="flex flex-col gap-6 pb-8 pt-4 w-full items-center overflow-y-auto h-full ac-scrollbar"
  >
  ...
  {#if isEditing}
    <div
      class="bg-[#f0ece0] rounded-2xl p-4 border border-[#e4dfd0] flex flex-col gap-3 animate-fade-in"
    >
      <h2 class="font-bold text-xs text-gray-500 uppercase tracking-wider mb-1">
      ...
      <div class="flex flex-col gap-1">
        <label
          for="passport-name-input"
          class="text-xs font-semibold text-gray-600">Resident Name</label
        >
        <input
      ...
      <div class="flex flex-col gap-1">
        <label
          for="passport-island-input"
          class="text-xs font-semibold text-gray-600">Island Name</label
        >
        <input
      ...
      <div class="flex flex-col gap-1">
        <label
          for="passport-fc-input"
          class="text-xs font-semibold text-gray-600">Dream Address</label
        >
        <input
      ...
      <div class="flex flex-col gap-1">
        <label
          for="passport-title-prefix-select"
          class="text-xs font-semibold text-gray-600">Passport Title</label
        >
      ...
      <div class="flex flex-col gap-1">
        <label
          for="passport-comment-input"
          class="text-xs font-semibold text-gray-600">Passport Comment</label
        >
        <textarea
  ...
  {#if dialogType !== "none"}
    <div
      class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in pointer-events-auto"
    >
      <AcnhBubble
        title="Nook Service"
        badgeBg="#5c9c6f"
        badgeColor="#ffffff"
        dialogText={dialogType === "need-pro"
          ? "You need a PRO account to create and sync multiple passports across devices!"
          : dialogType === "min-limit"
            ? "You must have at least one passport!"
            : dialogType === "confirm-delete"
              ? "Are you sure you want to delete this passport? This action cannot be undone."
              : dialogType === "delete-failed"
                ? "Failed to delete passport from the server. Please try again."
                : ""}
        isActive={true}
      >
        <div
          class="mt-4 border-t border-[#e1d9be]/60 pt-4 flex gap-2 w-full animate-fade-in relative z-50"
        >
          {#if dialogType === "confirm-delete"}
            <button
              onclick={async () => {
                if (deleteTargetId !== null) {
                  ...
                  dialogType = "none";
                  deleteTargetId = null;
                  await proceedDelete(target);
                }
              }}
              class="flex-[2] border-0 bg-[#ab3b3b] text-white px-4 py-2.5 rounded-xl font-bold shadow-sm hover:bg-[#8f3131] active:scale-95 transition-transform text-center cursor-pointer"
            >
              Delete
            </button>
            <button
